...
(function($){
        ...
        jQuery(document).ready(
            function () {
                ...
                if (!jQuery('#ptTimeSelectCntr').length) {
                    //NOTE: Updated for hours to pad with zero - HVB
                    jQuery("body").append(
    ...
    jQuery.ptTimeSelect.openCntr = function (ele) {
        ...
        //NOTE: Added below block for popup to show on top - HVB
        // Block Added Start
        var popupHeight = cntr.outerHeight();
        if ((style.top - popupHeight) > 0) {
            var adj = 7;
            if(i.hasClass('input-xs')){ adj = 0; }
            style.top = style.top - (popupHeight + adj);
        }
        // Block Added End
        ...
        cntr.css(style);
        var hr    = 12; //NOTE: Updated for default to 12 AM - HVB
        var min   = '00';
        var tm    = 'AM';
        if (i.val()) {
            ...
            if (match) {
                hr    = match[1] || 12;  //NOTE: Updated for default to 12 AM - HVB
                min    = match[2] || '00';
                tm    = match[3] || 'AM';
            }
        }
